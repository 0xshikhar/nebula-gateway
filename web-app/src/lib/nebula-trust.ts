...
export interface TrustGateCondition {
...
function buildSignalBreakdown(input: TrustInput, policy: TrustPolicyProfile): TrustSignalBreakdown {
  ...
  const conditions: TrustGateCondition[] = [
    {
      key: "wallet",
...
function collectFailureReasons(breakdown: TrustSignalBreakdown, policy: TrustPolicyProfile): string[] {
  ...
  if (!breakdown.conditions[0]?.passed) {
    reasons.push("Wallet address is required before verification.")
  }

  if (!breakdown.conditions[1]?.passed) {
    reasons.push("Proof expired before verification.")
  }

  if (!breakdown.conditions[2]?.passed) {
    reasons.push("Human proof is required for this policy.")
  }

  if (!breakdown.conditions[3]?.passed) {
    reasons.push("Credential verification is required for this policy.")
  }

  if (!breakdown.conditions[4]?.passed) {
    reasons.push(`Minimum band ${policy.minBand} is required.`)
  }

  if (!breakdown.conditions[5]?.passed) {
    reasons.push(`Minimum trust score ${policy.minTrustScore} is required.`)
  }

  if (!breakdown.conditions[6]?.passed) {
    reasons.push("Cohort membership is required for this policy.")
  }
...
export function evaluateTrust(input: TrustInput): TrustResult {
  ...
  const canReview =
    breakdown.trustScore >= policy.reviewThreshold &&
    breakdown.conditions[0]?.passed &&
    breakdown.conditions[1]?.passed
